...
type Props = {
  trades: ClosedTrade[];
  /** Prefer full history for inception; may be same as trades */
  allTrades?: ClosedTrade[];
};

export const PnLChart = memo(function PnLChart({ trades, allTrades }: Props) {
  const [period, setPeriod] = useState<PeriodKey>('all');
  const sourceAll = allTrades && allTrades.length ? allTrades : trades;

  const inceptionPnl = useMemo(
    () => sourceAll.reduce((s, t) => s + tradePnl(t), 0),
    [sourceAll]
  );

  const periodTrades = useMemo(() => filterTradesByPeriod(sourceAll, period), [sourceAll, period]);
  ...
  return (
    <section className="desk-card mb-3.5 p-4">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="desk-label mb-1">Realized P&amp;L</h2>
          <div className="flex flex-wrap items-baseline gap-4">
            <div>
              <div className="desk-label">Since inception</div>
            ...
            <div className="text-[0.75rem] text-desk-muted">
              {periodTrades.length} close{periodTrades.length === 1 ? '' : 's'}
            </div>
          </div>
        </div>
